        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-8 scrollbar-hide">
          <div className="space-y-1">
            {Object.keys(GROUP_CONFIG).map((group) => (
              <button 
                key={group} 
                onClick={() => { setActiveTab(group); setSelectedAgentId(null); setSidebarOpen(false); }} 
                className={`w-full flex items-center gap-4 px-4 py-3 md:py-4 rounded-xl text-xs md:text-sm font-black tracking-widest transition-all duration-300 border-y border-r border-transparent border-l-4 ${activeTab === group ? "bg-[#0a0a0a]" : "text-gray-500 hover:bg-white/5 hover:text-white hover:translate-x-2"}`} 
                style={{ 
                  color: activeTab === group ? (GROUP_CONFIG as any)[group].color : "", 
                  borderLeftColor: activeTab === group ? (GROUP_CONFIG as any)[group].color : "transparent",
                  boxShadow: activeTab === group ? `0 0 30px ${(GROUP_CONFIG as any)[group].color}33` : "",
                  textShadow: activeTab === group ? `0 0 10px ${(GROUP_CONFIG as any)[group].color}` : "" 
                }}
              >
                <span className="text-xl md:text-2xl drop-shadow-lg">{(GROUP_CONFIG as any)[group].icon}</span>
                <span className="truncate uppercase mt-0.5">{(GROUP_CONFIG as any)[group].label}</span>
              </button>
            ))}
          </div>
        ...
        <div className="p-4 md:p-10 space-y-6 md:space-y-10 animate-in fade-in duration-500">
          ...
          {activeTab !== "traffic" && !selectedAgent && (
             ...
             {Object.values(AGENT_REGISTRY).filter((a: any) => a.cat === activeTab).map((agent: any) => {
                const isActive = isExecuting && selectedAgentId === agent.id;
                return (
                  <button 
                    key={agent.id} 
                    onClick={() => setSelectedAgentId(agent.id)} 
                    className={`glass-card p-8 md:p-10 text-left transition-all group relative overflow-hidden shadow-xl ${
                      isActive ? 'border-cyan-500 shadow-[0_0_30px_rgba(0,242,255,0.3)] animate-pulse' : 'hover:border-cyan-500/40'
                    }`}
                  >
                    <div className="absolute -right-6 -bottom-6 w-64 h-64 bg-cover bg-center opacity-5" style={{ backgroundImage: `url('${agent.img}')` }}></div>
                    <div className={`w-16 h-16 rounded-2xl mb-6 bg-cover bg-center border border-white/10 shadow-lg relative ${isActive ? 'neon-ignition' : ''}`} style={{ backgroundImage: `url('${agent.img}')` }}>
                      {isActive && <div className="absolute inset-0 rounded-2xl border-2 border-cyan-400 animate-ping"></div>}
                    </div>
                    <h3 className={`text-lg md:text-xl font-black uppercase italic tracking-tighter mb-4 ${isActive ? 'text-cyan-400' : 'text-white'}`}>{agent.name}</h3>
                    <p className="text-[9px] text-gray-500 font-bold mb-8 uppercase tracking-widest leading-relaxed line-clamp-2">{agent.faz}</p>
                    <div className="mt-8 flex items-center gap-3 text-[10px] font-black uppercase text-cyan-400 italic">
                      {isActive ? 'Agente em Operação...' : 'Ativar Agente'} <ChevronRight size={14} className={isActive ? 'animate-bounce' : ''} />
                    </div>
                  </button>
                );
              })}
             ...
             <div className="space-y-8 md:space-y-12 animate-in zoom-in-95 duration-700">
               <div className="flex flex-col md:flex-row items-center gap-6 md:justify-between border-b border-white/5 pb-8 md:pb-10">
                <div className="flex flex-col md:flex-row items-center gap-6 md:gap-10 text-center md:text-left">
                  <div className={`w-24 h-24 md:w-32 md:h-32 bg-white/5 rounded-[35px] md:rounded-[45px] flex items-center justify-center border border-white/10 shadow-2xl relative bg-cover bg-center ${isExecuting ? 'neon-ignition border-cyan-500/50' : ''}`} style={{ backgroundImage: `url('${selectedAgent.img}')` }}>
                    {isExecuting && <div className="absolute inset-0 rounded-[45px] border-4 border-cyan-500 animate-ping opacity-30" />}
                  </div>
                  <div>
