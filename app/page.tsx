              ...
              <div className="flex flex-col gap-6 overflow-y-auto pr-2 scrollbar-hide">
                <div className="p-6 glass-card bg-white/[0.03] border-cyan-500/10 shadow-2xl">
                  ...
                  <div className="space-y-8">
                    {[{ label: "O QUE FAZ", text: selectedAgent.f, color: "bg-cyan-500", textColor: "text-cyan-400" }, { label: "O QUE RESOLVE", text: selectedAgent.r, color: "bg-[#00ff88]", textColor: "text-[#00ff88]" }, { label: "ONDE USAR", text: selectedAgent.u, color: "bg-[#7000ff]", textColor: "text-[#7000ff]" }].map((box, i) => (
                      <div key={i} className="space-y-3 group">
                        <div className="flex items-center gap-3"><div className={`w-1 h-4 ${box.color} rounded-full`} /><span className={`text-[9px] font-black uppercase tracking-[3px] ${box.textColor}`}>{box.label}</span></div>
                        <div className="p-5 rounded-2xl bg-white/5 border border-white/5 group-hover:border-white/10 transition-all"><p className="text-[11px] font-bold text-gray-300 leading-relaxed uppercase">{box.text}</p></div>
                      </div>
                    ))}
                  </div>
                </div>
